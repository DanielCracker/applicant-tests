...
--6) Найти дубликаты
--Во время передачи данных произошел сбой, в таблице orders появилось несколько дубликатов (несколько результатов возвращаются для date_time + customer_id + item_id). Вы должны их найти и вернуть количество дубликатов.
SELECT * FROM (
...
--Создать запрос, который найдет всех "важных" покупателей, т.е. тех, кто совершил наибольшее количество покупок после своего первого заказа.
...
--Написать запрос, который найдет всех клиентов, у которых суммарная выручка за последний месяц превышает среднюю выручку за все месяцы.
...
FROM(
	...
	SUM(IIF(EXTRACT(MONTH FROM ORDERS.DATE_TIME) = EXTRACT(MONTH FROM CURRENT_DATE) AND 
		EXTRACT(YEAR FROM ORDERS.DATE_TIME) = EXTRACT(YEAR FROM CURRENT_DATE),ITEMS.QUANTITY * ITEMS.ITEM_PRICE,0)) OVER (PARTITION BY СUSTOMER.CUSTOMER_ID) last_month,
	AVG(ITEMS.QUANTITY * ITEMS.ITEM_PRICE) OVER (PARTITION BY СUSTOMER.CUSTOMER_ID) as total_avg_Revenue,
	SUM(ITEMS.QUANTITY * ITEMS.ITEM_PRICE) OVER (PARTITION BY СUSTOMER.CUSTOMER_ID) as total_Revenue
	FROM ORDERS
	LEFT JOIN СUSTOMER ON ORDERS.CUSTOMER_ID = ORDERS.CUSTOMER_ID
	LEFT JOIN ITEMS ON ORDERS.ITEM_ID = ITEMS.ITEM_ID
	)x
	WHERE X.last_month > total_avg_Revenue